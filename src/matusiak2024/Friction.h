...
template<typename T> FrictionValue<T> Evaluate(FrictionParameters<T> p, T normal_force, T z, T v) {
    ...
    const T e = p.Exponential ? exp(-abs(v) / p.StribeckVelocity) : exp(-v * v / (p.StribeckVelocity * p.StribeckVelocity));
    const T ss = (fc + (fs - fc) * e) / p.Stiffness;
    const T dss = (fs - fc) * e / p.Stiffness * (p.Exponential ? -sign_v / p.StribeckVelocity : -2 * v / (p.StribeckVelocity * p.StribeckVelocity));
    ...
    const T damping = p.Damping, dd = 0;
