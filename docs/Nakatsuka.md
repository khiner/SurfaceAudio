...
Published settings leave exact reproduction unresolved; the listening page labels the reconstructed scenes accordingly.
...
These comparisons provide audible context; they do not establish perceptual reproduction of the author results.
...
The scalar amplitude is the magnitude of the vertex displacement; the paper prints a vector displacement without specifying a scalar projection.
...
Output amplitudes therefore represent an uncalibrated pressure-like signal.
...
`AdhesionCorrection(..., false)` preserves that printed behavior, which is also the default scene convention.
The reconstructed scenes explicitly select the shifted residual `Z/r - Z/d`, whose zero lies at the adhesion boundary.
...
The shifted residual and attachment rules are reconstruction choices; the paper supplies no executable contact solver.
...
These values remain explicit inputs rather than inferred physical measurements.
...
These diagnostics measure selected differences; perceptual equivalence remains unvalidated.
