...
Those inputs cover four materials with unknown sizes and an unverified connection to the 2026 survey.

## Run

After building, replay the fitted responses and generate impacts and statistical samples:
...
FIRs use the 2023 reconstruction conventions: centered Hamming-windowed sinc filters with unit L2 norm.
...
Covariance uses 1/(n-1) and preserves zero-variance dimensions.
...
Discrete convolution preserves digital gain.
...
The glass cohort is omitted because its highest noise band has only one observation after the published decay exclusion.
...
Audio callback deadlines and sound-pressure calibration are unverified.
